...
@section('content')
  ...
  <div class="bungkus1">
    ...
    <div class="menu" style="width: 16rem; height:23rem;">
      <img src="{{ asset('img/' . $pcg2->foto) }}" alt="{{ $pcg2->nama }}">
      <div class="menubody">
        <p class="nama">{{$pcg2->nama}}</p>
        <p class="harga">{{$pcg2->harga}}</p>
        <a href="/pancongs2/{{$pcg2->id_menu}}" class="read">Readmore</a>
      </div>
    </div>
    @endforeach
